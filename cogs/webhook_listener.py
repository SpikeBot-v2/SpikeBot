    ...
    async def on_message(self, message: discord.Message):
        ...
        try:
            ...
            if not hmac.compare_digest(expected_signature, hmac_signature):
                print(f"Invalid HMAC signature received in channel {message.channel.id}")
                return
            ...
            async with async_session() as session:
                ...
                try:
                    ...
                    if cookies_str:
                        ...
                        print("Neither cookies nor access token present in payload.")
                        return

                    puuid, riot_id = await api.get_user_info(access_token)
                except Exception as e:
                    print(f"Riot API authentication failed for user {user_id}: {e}")
            ...
            print(f"Successfully processed authentication for user {user_id} with Riot ID {riot_id}")

        except Exception as e:
            print(f"An error occurred in on_message webhook processing: {e}")
